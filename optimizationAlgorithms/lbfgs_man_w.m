function out = lbfgs_w(data,w0,mu0,X0,N,K,D,maxiter,myrlbfgs)
    ...
    symposM = sympositivedefinitefactory(D);
    symposM.transp = symposM.paralleltransp;
    struct1 = struct('Sigma', powermanifold(symposM,K),'mu',euclideanfactory(K,D), 'w', euclideanfactory(1,K-1));
    ...
    function grad_struct = grad_negloglikelihood(varstruct)
        ...
        for i=1:N
            ...
            for j=1:K
                Sigma_grad{j} = Sigma_grad{j} - (p(j)*grad_S(data(i,:)-mu(j,:),Sigma{j},mvns(i,j))/sum_j);
                mu_grad(j,:) = mu_grad(j,:) - p(j)*grad_mu2(data(i,:),mu(j,:), Sigma{j}, mvns(i,j))/sum_j;
            end 
            for j=1:(K-1)
        ...
        grad_struct = struct('Sigma',{Sigma_grad},'mu',mu_grad, 'w', w_grad);     
    end
    ...
    x0 = struct("Sigma",{Sigma0},"mu",mu0,"w",w0);
