function [x, cost, info, options,singular,lsfailed] = my_rlbfgs(problem, x0, options)
...
%       reduced the default value of storedepth from 30 to 2 as a result.
    ...
    while true
        ...
        %disp("grad")
        df0=M.inner(xCur, xCurGradient, p);
        if df0 > 0 
            stepsize = 0;
            xNext = xCur;
            newkey = key;
        ...
        % Execute line-search
            [stepsize, xNext, lsmem, newkey, lsstats,singular] = ...
                my_linesearch(problem, xCur, p, xCurCost, ...
                                df0, ...
                                options, storedb, lsmem,xCurGradient,key);
        end 
        if lsstats.costevals>19
            lsfailed=true;
        else 
            lsfailed=false;
        end
