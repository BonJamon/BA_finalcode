function [stepsize newx lsmem new_key lsstats singular] = ...
       my_linesearch(problem, x, d, f0, df0, options, storedb, lsmem, grad,key)
...
% Inpired by the linesearch code of minFunc toolbox
    ...
    if init_step_proc == 1%ALERT:new_alpha can be 0
        if isfield(lsmem, 'f0')
    ...
    if first_run
        new_df = -1;
        new_f = getCost(problem, newx, storedb,new_key);
    else
        [new_f new_grad] = getCostGrad(problem, newx, storedb,new_key);
        if ~is_illegal(new_f)
    ...
    while ls_iter < max_ls_steps
        ...
        if is_illegal(new_f)
            ...
            while true
                ...
                if new_f <= f0 + suff_decr*new_alpha*df0
                    new_grad = getGradient(problem, newx, storedb,new_key);
        ...
        alpha = new_alpha;
        if new_alpha~=0%TODO: Is that okay?
